...
{
    public partial class PairGameEasyView : Window
    {
        private readonly GameRecordManager _gameRecordManager;

        #region Constructors

        public PairGameEasyView(Player player)
        {
            InitializeComponent();
            ViewModel.Score = 0;
        ...
        public PairGameEasyViewModel ViewModel => DataContext as PairGameEasyViewModel;
        ...
        private void NewGame_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (ViewModel.State == StateOfGame.GameOver)
            {
                ...
                for (int i = 0; i < 16; i++) gameGrid.Children[i].Visibility = Visibility.Visible;
        ...
        private void Card_MouseDown(object sender, MouseButtonEventArgs e)
        {
            if (ViewModel.Time > 0 && ViewModel.State == StateOfGame.Running)
            {
                ...
                if (int.TryParse(nameOfTheCard, out int indexOfTheCard) == false)
        ...
        private void Wait(long milliseconds)
        {
            ...
            {
                Grid grid = new Grid();
                grid.Dispatcher.Invoke(DispatcherPriority.Background,
        ...
        private void Time_Click(object sender, RoutedEventArgs e)
        {
            if (ViewModel.State == StateOfGame.GameOver)
            {
                TimeWindow timeWindow = new TimeWindow(ViewModel);
                timeWindow.Show();
            }
            else
            {
